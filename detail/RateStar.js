...
import { reviewProduct , deleteReview, getproductById} from '../../actions/ProductAction';
...
function RateStar(props) {
    ...
    const updatedProduct = useSelector(state => state.getProductById.product);
    ...
    const onDeleteReview = async (reviewId) => {
        if (window.confirm('Bạn có chắc chắn muốn xóa đánh giá này?')) {
          await dispatch(deleteReview(id, reviewId)); // Xóa đánh giá từ store
    ...
    return (
        ...
        <div className="">
                ...
                <div className="rate-info">
                    <Row>
                        <Col span={7} style={{display:'flex', alignItems:'center', justifyContent:'center', flexDirection:'column'}}>
                            ...
                            <p className="star-average-numz" style={{marginBottom: 0, fontSize: '25px', color: 'orange'}}>
          ...
          <Col span={18} align="start" xs={24} sm={24} md={18} key={item._id}>
            <div className="danhgia">
              <p className="name" style={{ fontWeight: 'bold', fontSize: '15px' }}>
                {item.name}
                {existsUser && existsUser._id === item._id ? (
                  <span
              ...
              <div className="cmt" style={{ display: 'flex' }}>
                <Rate style={{ color: 'orange', fontSize: '14px' }} value={item.star} disabled />
